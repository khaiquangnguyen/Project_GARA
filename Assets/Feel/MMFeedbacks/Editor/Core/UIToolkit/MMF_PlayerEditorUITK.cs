...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_PlayerEditorUITK : Editor
	{
		...
		protected const string _changeDurationUndoText = "Change Feedback Duration";
		...
		protected const string _runFirstOnEnableOnlyAfterStart = "RunFirstOnEnableOnlyAfterStart";
		...
		protected virtual void DrawSettingsDropDown(VisualElement root)
		{
			...
			void BuildSettingsInitializationFoldout()
			{
				...
				MMUIToolkit.CreateAndBindPropertyField(_logMissingTargetsPropertyName, serializedObject,
					settingsInitializationFoldout);
				MMUIToolkit.CreateAndBindPropertyField(_runFirstOnEnableOnlyAfterStart, serializedObject,
					settingsInitializationFoldout);
			}
			...
			FeedbackHeaderContainersData feedbackHeaderContainersData)
		{
				...
				if (TargetMmfPlayer.FeedbacksList[index].Timing.NumberOfRepeats != 0)
				{
					float delayBetweenRepeats =
						TargetMmfPlayer.ApplyTimeMultiplier(TargetMmfPlayer.FeedbacksList[index].Timing
							.DelayBetweenRepeats);
					bool ignoreFeedbackDurationForRepeats = TargetMmfPlayer.FeedbacksList[index].Timing.IgnoreFeedbackDurationForRepeats;
					string repeatedTerm = ignoreFeedbackDurationForRepeats
						? delayBetweenRepeats + "s"
						: TargetMmfPlayer.FeedbacksList[index].FeedbackDuration.ToString() + "s + " + delayBetweenRepeats + "s";
			...
			int clickCount = 0;
			long lastClickTime = 0;
			const long doubleClickTimeMs = 300;
			timingLabel.RegisterCallback<MouseDownEvent>(evt =>
			{
				if (evt.button != 0)
				{
					return;
				...
				if (currentTime - lastClickTime < doubleClickTimeMs)
				{
					...
					{
						evt.StopPropagation();
						ProcessEditFeedbackDuration(index, timingLabel, feedbackHeaderContainersData.HeaderContainer);
						clickCount = 0;
					}
				}
				else
		...
		/// <summary>
		/// Starts the duration edit process for a feedback by replacing the timing label with a text field in place
		/// </summary>
		protected virtual void ProcessEditFeedbackDuration(int index, Label timingLabel, VisualElement headerContainer)
		{
			...
			float initialDuration = feedback.FeedbackDuration;

			TextField durationField = new TextField();
			durationField.value = initialDuration.ToString("F2", CultureInfo.InvariantCulture);
			durationField.AddToClassList(_feedbackTimingLabelClassName);

			durationField.style.position = Position.Absolute;
			durationField.style.left = timingLabel.worldBound.x - headerContainer.worldBound.x;
			durationField.style.top = timingLabel.worldBound.y - headerContainer.worldBound.y;
			durationField.style.minWidth = Mathf.Max(80f, timingLabel.worldBound.width);
			durationField.style.maxWidth = 260f;
			durationField.style.height = timingLabel.worldBound.height;

			durationField.schedule.Execute(() =>
			{
				Label textFieldLabel = durationField.Q<Label>();
				if (textFieldLabel != null)
				{
					textFieldLabel.style.display = DisplayStyle.None;
				}

				VisualElement textInput = durationField.Q("unity-text-input");
				if (textInput != null)
				{
			...
			timingLabel.style.opacity = 0;
			headerContainer.Add(durationField);
			...
			void FinishEdit(bool save)
			{
				if (save)
				{
					string candidate = durationField.value.Trim().Replace(',', '.');
					if (float.TryParse(candidate, NumberStyles.Float, CultureInfo.InvariantCulture, out float parsedDuration))
					{
						float sanitizedDuration = Mathf.Max(0f, parsedDuration);
						Undo.RecordObject(target, _changeDurationUndoText);
						feedback.FeedbackDuration = sanitizedDuration;
						TargetMmfPlayer.ComputeCachedTotalDuration();
						SavePlayerChanges();
						DrawDurationAndDirectionContents();
						RedrawAllFeedbackHeaders();
					}
				...
				durationField.RemoveFromHierarchy();
				timingLabel.style.opacity = 1;
			}

			durationField.RegisterCallback<KeyDownEvent>(evt =>
			{
			...
			durationField.RegisterCallback<BlurEvent>(evt => { FinishEdit(true); });
		}
