...
namespace Unity.Pipeline.Editor.Commands.Materials
{
    ...
    static class MaterialCommands
    {
        ...
        public static MaterialPropertiesResult GetMaterialProperties(
            [CliArg("material",
                "Reference to the .mat asset (or a loaded material) to read (path / guid / globalId / instanceId).",
                Required = true)]
            ObjectRef material)
        {
            ...
            if (shader != null)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (propType == ShaderPropertyType.Range)
                    {
                        ...
                        entry.Range = new MaterialRange { Min = limits.x, Max = limits.y, };
                    }
        ...
        public static SetMaterialPropertiesResult SetMaterialProperties(
            [CliArg("material",
                "Reference to the .mat asset (or a loaded material) to edit (path / guid / globalId / instanceId).",
                Required = true)]
            ObjectRef material,
            [CliArg("shader",
                "Reassign the material's shader by name (e.g. \"Standard\", \"Universal Render Pipeline/Lit\", or a Shader Graph shader name). Applied before properties so new property names resolve against the new shader.")]
            string shader = null,
            [CliArg("properties",
                "JSON object of shader property name -> value. Names must include the leading underscore (e.g. _BaseColor). Float/Range/Int=number; Color=[r,g,b,a] or hex string; Vector=[x,y,z,w]; Texture=an object reference {guid/path} or null.")]
            JObject properties = null,
            [CliArg("renderQueue", "Explicit render queue, or -1 to inherit from the shader. Omit to leave unchanged.")]
            int? renderQueue = null,
            [CliArg("enableKeywords", "Shader keywords to enable (e.g. _NORMALMAP, _EMISSION).")]
            string[] enableKeywords = null,
            [CliArg("disableKeywords", "Shader keywords to disable.")]
            string[] disableKeywords = null,
            [CliArg("confirm",
                "Reserved for parity; editing an existing material is non-destructive and undoable, so it is not required.")]
            bool confirm = false,
            [CliArg("dry_run",
                "If true, validate the shader, resolve property names and texture refs, and report applied[]/unknown[] without writing anything.")]
            bool dryRun = false)
        {
            var mat = ResolveMaterial(material, out var assetPath);
            ...
            using (new AuthoringUndoScope("Set Material Properties"))
            {
                ...
                if (properties != null)
                {
                    foreach (var pair in properties)
                    {
                        ...
                        {
                            result.Unknown.Add(
                                $"{name}: unknown property (not declared by shader '{(mat.shader != null ? mat.shader.name : "<none>")}')");
                            continue;
                        }
                        ...
                        if (propType == null)
                        {
                            ...
                            result.Unknown.Add(
                                $"{name}: property is not a settable shader property on '{(mat.shader != null ? mat.shader.name : "<none>")}'");
                            continue;
                        }
        ...
        internal static Material ResolveMaterial(ObjectRef material, out string assetPath)
        {
            ...
            if (!(obj is Material mat))
                throw new ArgumentException(
                    $"Reference '{material}' resolved to a {obj.GetType().Name}, not a Material.");
        ...
        private static void ValidateProperties(Shader shader, JObject properties, SetMaterialPropertiesResult result)
        {
            ...
            foreach (var pair in properties)
            {
                ...
                {
                    result.Unknown.Add(
                        $"{name}: unknown property (not declared by shader '{(shader != null ? shader.name : "<none>")}')");
                    continue;
                }
