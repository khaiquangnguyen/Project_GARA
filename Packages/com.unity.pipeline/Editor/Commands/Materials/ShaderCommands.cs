...
namespace Unity.Pipeline.Editor.Commands.Materials
{
    ...
    static class ShaderCommands
    {
        ...
        public static List<ShaderInfo> ListShaders(
            [CliArg("filter", "Case-insensitive substring matched against the shader name (e.g. \"URP\", \"Lit\").")]
            string filter = null,
            [CliArg("includeBuiltin",
                "Include built-in/engine shaders (those with no project asset path). Default true.")]
            bool includeBuiltin = true,
            [CliArg("limit", "Maximum number of shaders to return (default 200).")]
            int limit = 200)
        {
            if (limit <= 0)
                limit = 200;
            ...
            foreach (var guid in AssetDatabase.FindAssets("t:Shader"))
            {
                ...
                {
                    Name = shader.name, AssetPath = path, IsBuiltin = false, IsSupported = shader.isSupported,
                });
            ...
            if (includeBuiltin)
            {
                ...
                foreach (var info in ShaderUtil.GetAllShaderInfo())
                {
                    ...
                    results.Add(new ShaderInfo
                    {
                        Name = info.name, AssetPath = null, IsBuiltin = true, IsSupported = info.supported,
                    });
        ...
        public static ShaderPropertiesResult GetShaderProperties(
            [CliArg("shader", "Shader name (e.g. \"Universal Render Pipeline/Lit\"). Provide this OR 'material'.")]
            string shader = null,
            [CliArg("material",
                "Reference to a material to read the shader from instead of naming it. Provide this OR 'shader'.")]
            ObjectRef material = null)
        {
            Shader resolved;
            ...
            else
            {
                throw new ArgumentException(
                    "Provide either 'shader' (a shader name) or 'material' (a material reference).");
            }
