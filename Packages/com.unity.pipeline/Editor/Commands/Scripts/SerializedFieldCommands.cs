...
namespace Unity.Pipeline.Editor.Commands.Scripts
{
    ...
    static class SerializedFieldCommands
    {
        ...
        public static AuthoringResult SetSerializedField(
            [CliArg("target",
                "Reference to the component or asset to modify (globalId/path/guid/instanceId/hierarchyPath). May be a GameObject when 'component' is given.",
                Required = true)]
            ObjectRef target,
            [CliArg("field", "SerializedProperty path, e.g. 'speed', 'settings.speed', or 'waypoints.Array.data[0]'.",
                Required = true)]
            string field,
            [CliArg("value",
                "JSON value to assign. For object references pass an ObjectRef object (or null to clear). For enums pass the value name.",
                Required = true)]
            JToken value,
            [CliArg("component",
                "Component type name on the target GameObject (e.g. 'Rigidbody'). Use when 'target' is a GameObject; omit when 'target' is already a component handle.")]
        ...
        public static object GetSerializedFields(
            [CliArg("target",
                "Reference to the component or asset to read (globalId/path/guid/instanceId/hierarchyPath). May be a GameObject when 'component' is given.",
                Required = true)]
            ObjectRef target,
            [CliArg("field",
                "Optional single SerializedProperty path to read (e.g. 'speed' or 'items.Array.data[0]'). Omit to read all top-level fields.")]
            string field = null,
            [CliArg("component",
                "Component type name on the target GameObject (e.g. 'Rigidbody'). Use when 'target' is a GameObject; omit when 'target' is already a component handle.")]
            string component = null,
            [CliArg("format",
                "Output shape. Omit (default) for a full descriptor per field {name,path,propertyType,isArray,arrayLength,value}; 'value' returns just the value(s): the raw value for a single 'field', or a {name: value} map for all fields.")]
            string format = null)
        {
            if (target == null || target.IsEmpty)
            ...
            if (!string.IsNullOrWhiteSpace(field))
            {
                ...
                return new { type = obj.GetType().Name, fields = new[] { DescribeProperty(prop) } };
            }
            ...
            while (iterator.NextVisible(enterChildren))
            {
                ...
                if (valueOnly)
                {
                    ...
                    try
                    {
                        values[iterator.name] = SerializedPropertyConverter.GetValue(iterator);
                    }
                    catch (Exception ex)
                    {
                        values[iterator.name] = $"<error:{ex.Message}>";
                    }
                }
                else
                {
        ...
        private static Object ResolveSerializable(ObjectRef target, string component = null)
        {
            ...
            if (obj is GameObject go)
            {
                ...
                if (matches.Length > 1)
                {
                    ...
                    sb.Append(
                            $"GameObject '{go.name}' has {matches.Length} components of type '{componentType.Name}'. ")
