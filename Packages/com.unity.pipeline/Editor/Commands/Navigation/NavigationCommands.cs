...
namespace Unity.Pipeline.Editor.Commands.Navigation
{
    ...
    static class NavigationCommands
    {
        ...
        public static SelectionResult SetSelection(
            [CliArg("instance_ids", "Scene/loaded object instance IDs to select.")]
            ObjectId[] instanceIds = null,
            [CliArg("paths", "Asset paths to select (e.g. Assets/Foo.prefab).")]
            string[] paths = null)
        {
            ...
            if (resolved.Count == 0 && hadInputs)
                throw new ArgumentException(
                    $"No objects resolved from the given inputs. Unresolved: {string.Join(", ", unresolved)}");
        ...
        public static SearchResult Search(
            [CliArg("query", "Unity Search query string, e.g. 't:Material', 'p: my asset', 'h: Main Camera'.",
                Required = true)]
            string query,
            [CliArg("limit", "Max results to return (capped 200).")]
            int limit = 50)
        {
            ...
            if (clamped <= 0)
            {
                return new SearchResult { Query = query, Count = 0, Results = Array.Empty<SearchResultItem>() };
            }
            ...
            return new SearchResult { Query = query, Count = results.Count, Results = results.ToArray() };
        }
