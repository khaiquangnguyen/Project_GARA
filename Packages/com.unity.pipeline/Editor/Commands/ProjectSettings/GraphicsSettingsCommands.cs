...
namespace Unity.Pipeline.Editor.Commands.ProjectSettings
{
    ...
    static class GraphicsSettingsCommands
    {
        ...
        [CliCommand("get_graphics_settings", "Read GraphicsSettings (default render pipeline).",
            MainThreadRequired = true, Tags = new[] { "settings/graphics" })]
        public static ProjectSettingsResponse Get() => ProjectSettingsCommand.Get(Group, Read);

        [CliCommand("set_graphics_settings",
        ...
        public static ProjectSettingsResponse Set(
            [CliArg("settings", "Fields to change; omitted fields are left unchanged.")]
            GraphicsSettingsInput settings = null,
            [CliArg("confirm", "Apply the change. Without it the call is refused.")]
            bool confirm = false,
            [CliArg("dry_run", "Preview the change without applying it.")]
            bool dryRun = false)
        {
            if (settings == null)
            ...
            if (!settings.RenderPipelineAsset.IsEmpty)
            {
                ...
                if (asset == null)
                    return ProjectSettingsResponse.Fail(Group,
                        $"'{settings.RenderPipelineAsset}' is not a RenderPipelineAsset.");
            }
    ...
    {
        [CliArg("renderPipelineAsset",
            "Reference (path / guid / globalId) to a RenderPipelineAsset to set as the default. Pass an empty reference ({}) to select the built-in pipeline; omit to leave unchanged.")]
