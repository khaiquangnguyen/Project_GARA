...
namespace Unity.Pipeline.Editor.Commands.PackageManager
{
    ...
    static class PackageIdentifier
    {
        ...
        public static bool TryParse(string identifier, out ParsedPackageId parsed, out string error)
        {
            ...
            {
                error =
                    "Package identifier is empty. Provide a name (com.unity.foo[@version]), a git URL, or a file: path.";
                return false;
            }
            ...
            switch (Classify(id))
            {
                case PackageSourceKind.Local:
                    ...
                    {
                        Kind = PackageSourceKind.Local, Identifier = id, Description = $"local package '{id}'"
                    };
                    return true;
