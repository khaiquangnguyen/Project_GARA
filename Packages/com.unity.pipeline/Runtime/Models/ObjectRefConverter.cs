...
namespace Unity.Pipeline.Models
{
    ...
    {
        private static readonly Regex s_GuidWithFileId =
            new Regex(@"^guid:([0-9a-fA-F]+):([0-9]+)$", RegexOptions.Compiled);
        ...
        public override void WriteJson(JsonWriter writer, ObjectRef value, JsonSerializer serializer)
        {
            ...
            if (value.FileId.HasValue) writer.WriteValue(value.FileId.Value);
            ...
            if (value.InstanceId.HasValue) serializer.Serialize(writer, value.InstanceId.Value);
            else writer.WriteNull();
