...
namespace Unity.Pipeline.Runtime.Commands
{
    ...
    static class ConsoleCommand
    {
        ...
        [CliCommand("console",
            ...
            MainThreadRequired = false, Tags = new[] { "observability/console" })]
        public static ConsoleLogResponse GetConsole(
            [CliArg("tail", "Maximum number of most-recent entries to return")]
            int tail = DefaultTail,
            [CliArg("level", "Minimum severity to include: log | warn | error")]
            string level = ConsoleLogBuffer.LevelLog,
            [CliArg("since",
                "Cursor: only return entries newer than this seq. Use the 'cursor' from a previous response to follow.")]
            long since = -1,
            [CliArg("since_session",
                "Session the 'since' cursor came from: pass the 'session' from a previous response. A cursor from another session is refused and the tail returned with reset=true.")]
            string since_session = null)
        {
        ...
        [CliCommand("console_status",
            "Console ground truth and buffer counters without pulling entries: compile-failure flag, Editor console counts, and the buffer's retained counts and cursor.",
            MainThreadRequired = false, Tags = new[] { "observability/console" })]
        ...
        [CliCommand("clear_console", "Clear the captured log buffer and the Unity Editor console.",
            Tags = new[] { "observability/console" })]
