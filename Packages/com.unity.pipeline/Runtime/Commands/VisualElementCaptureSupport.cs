...
namespace Unity.Pipeline.Runtime.Commands
{
    ...
    static class VisualElementCaptureSupport
    {
        ...
        static bool TryApplyPseudo(ref UQueryBuilder<VisualElement> query, string name, bool negate)
        {
            ...
            {
                case "checked":
                    query = on ? query.Checked() : query.NotChecked();
                    return true;
                case "hover":
                    query = on ? query.Hovered() : query.NotHovered();
                    return true;
                case "focus":
                    query = on ? query.Focused() : query.NotFocused();
                    return true;
                case "active":
                    query = on ? query.Active() : query.NotActive();
                    return true;
                case "enabled":
                    query = on ? query.Enabled() : query.NotEnabled();
                    return true;
                case "disabled":
                    query = on ? query.NotEnabled() : query.Enabled();
                    return true;
