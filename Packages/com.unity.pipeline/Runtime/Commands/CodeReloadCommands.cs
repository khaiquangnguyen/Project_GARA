...
namespace Unity.Pipeline.Runtime.Commands
{
    ...
    {
        [CliCommand("reload_file", "Compile and apply in-place [CodeReload] edits from a source file",
            MainThreadRequired = true, Tags = new[] { "scripts/codereload" })]
        public static CodeReloadResponse ReloadFile(
            [CliArg("filename", "Source file containing [CodeReload] methods (e.g. Assets/Scripts/Player.cs)",
                Required = true)]
            string filename,
            [CliArg("timeout", "Compilation timeout in milliseconds")]
            int timeout = 30000,
            [CliArg("assemblyDir",
                "Directory to save compiled assemblies to disk (optional, default is in-memory only)")]
            string assemblyDir = null,
            [CliArg("pdb",
                "Emit debug symbols (portable PDB) mapped to the original source so breakpoints bind in your editor. Compiles unoptimized.")]
            bool pdb = false)
        {
            return ReloadFileCore("reload_file", filename, timeout, assemblyDir, pdb, useInterpreter: false);
        }

        [CliCommand("reload_file_editor_interpreter",
            "Compile in-place [CodeReload] edits and run them through the IlInterpreter VM in this process instead of Assembly.Load (IL2CPP-safe; only a minimal host API + the target type are available)",
            MainThreadRequired = true, Tags = new[] { "scripts/codereload" })]
        public static CodeReloadResponse ReloadFileEditorInterpreter(
            ...
            bool pdb = false)
        {
            return ReloadFileCore("reload_file_editor_interpreter", filename, timeout, assemblyDir, pdb,
                useInterpreter: true);
        }
        ...
        private static CodeReloadResponse ReloadFileCore(string commandName, string filename, int timeout,
            string assemblyDir, bool pdb, bool useInterpreter)
        {
            ...
            try
            {
                ...
                Debug.Log(
                    $"CodeReload: Executing {commandName} command: {fullPath}, timeout: {timeout}ms, assemblyDir: {assemblyDir ?? "in-memory"}, pdb: {pdb}, interpreter: {useInterpreter}");
        ...
        [CliCommand("cleanup_codereload", "Remove old code reload DLL versions and clear registry",
            MainThreadRequired = true, RuntimeOnly = true, Tags = new[] { "scripts/codereload" })]
        public static CodeReloadResponse CleanupCodeReload(
            [CliArg("assemblyDir", "Directory containing assemblies to cleanup", Required = true)]
            string assemblyDir,
            [CliArg("force_domain_reload", "Force Unity domain reload after cleanup")]
            bool forceDomainReload = true)
        {
            var stopwatch = System.Diagnostics.Stopwatch.StartNew();
            ...
            try
            {
                ...
                if (cleanupResult.Success)
                {
                    ...
                    Debug.Log(
                        $"CodeReload: cleanup_codereload completed successfully in {stopwatch.ElapsedMilliseconds}ms");
        ...
        [CliCommand("codereload_status", "Show current code reload registry status and statistics",
            MainThreadRequired = true, RuntimeOnly = true, Tags = new[] { "scripts/codereload" })]
        ...
        /// </summary>
        public static PathValidationResult ValidateReloadPath(string resolvedPath,
            System.Collections.Generic.IReadOnlyCollection<string> allowedRoots)
        {
        ...
        private static string ResolveSourceFilePath(string filename)
        {
            ...
            {
                Path.Combine("Assets", filename), Path.Combine("Assets", "Scripts", filename), filename // Project root
            };
    ...
    class CodeReloadResponse : CommandExecutionResponse
    {
        ...
        /// </summary>
        public static CodeReloadResponse CmdSuccess(string assemblyName, string message,
            System.Collections.Generic.List<string> items, long executionTimeMs)
        {
        ...
        /// </summary>
        public static CodeReloadResponse CmdFailure(string error, string errorDetails, long executionTimeMs,
