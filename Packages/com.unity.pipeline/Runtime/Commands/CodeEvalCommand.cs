...
namespace Unity.Pipeline.Runtime.Commands
{
    ...
    static class CodeEvalCommand
    {
        [CliCommand("eval", "Evaluate C# code dynamically using Roslyn compiler", MainThreadRequired = true,
            Tags = new[] { "scripts/eval" })]
        public static EvalResponse EvaluateCode(
            [CliArg("code", "C# code to evaluate", Required = true)]
            string code,
            [CliArg("timeout", "Timeout in milliseconds")]
            int timeout = 5000)
        {
        ...
        public static EvalResponse EvaluateFile(
            [CliArg("file", "Path to a .cs file to evaluate", Required = true)]
            string file,
            [CliArg("timeout", "Timeout in milliseconds")]
            int timeout = 5000)
        {
