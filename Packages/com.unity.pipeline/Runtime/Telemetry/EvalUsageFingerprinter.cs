    ...
    static class EvalUsageFingerprinter
    {
        ...
        public static EvalUsageAnalysis Analyze(string code)
        {
            if (string.IsNullOrWhiteSpace(code))
                ...
                {
                    Classification = EvalClassification.Statements, Fingerprints = new List<string>()
                };
            ...
            return new EvalUsageAnalysis { Classification = Classify(root), Fingerprints = ExtractFingerprints(root) };
        }
            ...
            public override void VisitConditionalAccessExpression(ConditionalAccessExpressionSyntax node)
            {
                ...
                if (node.WhenNotNull != null)
                {
                    foreach (var argumentList in node.WhenNotNull
                                 .DescendantNodes(n =>
                                     !(n is ArgumentListSyntax) && !(n is BracketedArgumentListSyntax))
        ...
        private static string RenderPath(ExpressionSyntax expr, int depth)
        {
            ...
            switch (expr)
            {
                ...
                case QualifiedNameSyntax qn
                    : // Namespace-qualified type in type position, e.g. new UnityEngine.GameObject
                ...
                case ObjectCreationExpressionSyntax oc
                    : // new GameObject("x").AddComponent<T> → new GameObject.AddComponent<T>
