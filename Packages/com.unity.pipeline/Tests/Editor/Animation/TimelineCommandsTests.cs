    ...
    class TimelineCommandsTests
    {
        ...
        public void Timeline_NotInstalled_ReturnsPackageNotFound()
        {
            if (TimelineGuard.IsInstalled())
                Assert.Pass(
                    "com.unity.timeline is installed; the not-installed path is covered only when it is absent.");
        ...
        public void Timeline_CreateTrackClipGet_Flow()
        {
            if (!TimelineGuard.IsInstalled())
                Assert.Pass(
                    "com.unity.timeline is not installed; the Timeline authoring flow is validated only when the package is present.");

            // create_timeline
            var timelinePath = Root + "/Cutscene.playable";
            var created = TimelineCommands.CreateTimeline(timelinePath, frameRate: 30f);
            Assert.IsInstanceOf<AuthoringResult>(created,
                "create_timeline should return an AuthoringResult when installed");
