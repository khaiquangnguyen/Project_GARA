...
namespace Unity.Pipeline.Tests.Editor.Capture
{
    ...
    class CaptureCommandsTests
    {
        ...
        public void CaptureGameView_SavePath_IncludeImage_ReturnsFileAndBase64()
        {
            ...
            var result = CaptureCommands.CaptureGameView(64, 64, CameraName, $"{SaveFolder}/both.png",
                includeInlineImage: true);
        ...
        {
            var ex = Assert.Throws<ArgumentException>(() => CaptureCommands.CaptureGameView(64, 64, source: "bogus"));
        ...
        public void CaptureGameView_ViaClient_Succeeds()
        {
            ...
            using (var server = new PipelineTestServer())
            {
                var response = server.Execute("capture_game_view",
                    new { width = 64, height = 64, camera = CameraName });
        ...
        public IEnumerator CaptureGameView_ScreenSource_CapturesOverlayUi()
        {
            ...
            if (!overlayObserved)
            {
                ...
                Assert.Ignore(
                    "Overlay not observable in this environment's game-view capture; screen path still verified.");
            }
