...
namespace Unity.Pipeline.Tests.Editor
{
    ...
    class EditorCodeReloadWatcherTests
    {
        ...
        public void StartWatch_File_SetsStateAndDisablesAutoRefresh()
        {
            ...
            Assert.AreEqual(0, EditorPrefs.GetInt(AutoRefreshPref, 1),
                "auto-refresh (legacy key) should be off while watching");
            Assert.AreEqual(0, EditorPrefs.GetInt(AutoRefreshModePref, 1),
                "auto-refresh mode (2021.2+ key) should be off while watching");
        }
        ...
        public void StopWatch_ClearsStateAndRestoresAutoRefresh()
        {
            ...
            Assert.AreEqual(1, EditorPrefs.GetInt(AutoRefreshPref, 1),
                "auto-refresh (legacy key) should be restored on stop");
            Assert.AreEqual(2, EditorPrefs.GetInt(AutoRefreshModePref, 1),
                "auto-refresh mode should be restored to its prior value (2), not just re-enabled");
        }
        ...
        public void StartWatch_MissingPath_DoesNotWatch()
        {
            ...
            EditorCodeReloadWatcher.StartWatch(Path.Combine(Path.GetTempPath(), "does_not_exist_x9.cs"),
                isFolder: false);
