...
namespace Unity.Pipeline.Tests.Editor.Assets
{
    ...
    class AssetImportCommandsTests
    {
        ...
        public void SetImportSettings_DryRun_DoesNotPersist()
        {
            ...
            CollectionAssert.Contains(result.Applied, "userData");
            Assert.AreNotEqual("dryrun", AssetImporter.GetAtPath(path).userData,
                "dry_run should not persist the change");
        }
        #endregion
        ...
        public void SetImportSettings_ViaClient_Succeeds()
        {
            ...
            {
                var response = server.Execute("set_import_settings",
                    new { asset = new { path }, settings = new { userData = "viaclient" } });
