...
namespace Unity.Pipeline.Tests.Editor
{
    ...
    class CodeReloadCompilerTests
    {
        ...
        public void CleanupCodeReloadDlls_WithVersionedDlls_KeepsOnlyLatestVersion()
        {
            ...
            Assert.GreaterOrEqual(result.ExecutionTimeMs, 0,
                "Should record execution time (0ms is acceptable for fast cleanup)");
        ...
        public void CleanupCodeReloadDlls_WithActiveOverrides_ClearsRegistry()
        {
            ...
            Assert.IsTrue(
                CodeReloadRegistry.RegisterMethodOverride(overrideMethod, overrideAttribute, typeof(CleanupOverrides)),
            ...
            // Assert
            Assert.IsTrue(result.Success, $"Cleanup should succeed. Message: {result.Message}");
            Assert.AreEqual(0, CodeReloadRegistry.GetStats().ActiveOverrideCount,
                "Registry should be cleared after cleanup");
        }
