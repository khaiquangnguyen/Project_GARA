...
namespace Unity.Pipeline.Tests.Editor
{
    ...
    {
        private const string TransientConfigAssetPath =
        ...
        [TestCase(true, false)]
        [TestCase(true, true)]
        public void Preprocess_PipelineNotShipping_LeavesNoTransientAssets(bool authorDisabledConfig,
            bool seedStaleAssets)
        {
        ...
        public void Preprocess_StaleTransientAssets_PipelineEnabled_ReplacesWithCurrentValues()
        {
            ...
            Assert.IsTrue(baked.enableInBuilds,
                "Baked asset must reflect the current authored config, not the stale leftover");
            Assert.AreEqual(7920, baked.port,
                "Baked asset must reflect the current authored config's port, not the stale leftover's 7999");
        ...
        public void BuildProcessor_ConfigEnabled_WritesTransientConfigAssetWithSameValues()
        {
            ...
            Assert.IsNotNull(baked,
                "OnPreprocessBuild should bake the authored config into a transient Resources asset");
            Assert.IsTrue(baked.enableInBuilds);
            Assert.AreEqual(7920, baked.port);
        }
    ...
    abstract class RuntimePipelineBuildTestBase
    {
        ...
        private const string TransientConfigAssetPath =
            "Assets/Settings/Pipeline/Resources/RuntimePipelineConfig.asset";
        private const string TransientBuildInfoAssetPath =
        ...
        public void BuildPlayerWithRuntimeConfig_ValidConfiguration_BuildSucceeds()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(File.Exists(buildOptions.locationPathName),
                    "Executable should exist after successful build");
            }
            finally
            {
