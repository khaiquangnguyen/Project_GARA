...
namespace Unity.Pipeline.Tests.Editor.Scripts
{
    ...
    class ScriptCommandsTests
    {
        ...
        public void SetArrayElement_ResizesAndWiresObjectReference_RoundTrips()
        {
            ...
            Assert.AreEqual(PipelineUtils.GetObjectId(m_RefTarget),
                elemJson["fields"][0]["value"]["instanceId"].ToObject<ObjectId>());
        }
        #endregion
        ...
        private static string FixtureScriptPath()
        {
            ...
            Assert.Ignore(
                "AttachByPathFixture.cs MonoScript not found in the AssetDatabase; cannot exercise attach-by-path.");
            return null;
        }
        ...
        public IEnumerator CreateScript_WritesFileAndReturnsAssetIdentity()
        {
            ...
            var result =
                CreateScriptCommand.CreateScript("CLI195Generated", k_CreateScriptTestFolder, "Game.Generated");
        ...
        public void GetSerializedFields_ViaClient_NullValue_ReturnsExplicitNullResult()
        {
            ...
            {
                var response = server.Execute("get_serialized_fields",
                    new
                    {
        ...
        public void SetThenGet_ViaClient_RoundTrips()
        {
            ...
            using (var server = new PipelineTestServer())
            {
                var setResponse = server.Execute("set_serialized_field",
                    new
                    {
                        target = new { instanceId = PipelineUtils.GetObjectId(comp) }, field = "m_Speed", value = 7
                    });
                Assert.IsTrue(setResponse.IsSuccess,
                    $"set should succeed: {setResponse.Error} / {setResponse.RawResponse}");
                Assert.AreEqual(7, comp.Speed);

                var getResponse = server.Execute("get_serialized_fields",
                    new { target = new { instanceId = PipelineUtils.GetObjectId(comp) }, field = "m_Speed" });
        ...
        public void AttachScript_ViaClient_UncompiledType_ReturnsErrorResponse()
        {
            using (var server = new PipelineTestServer())
            {
                ...
                var response = server.Execute("attach_script",
                    new
                    {
        ...
        public void AttachScript_ViaClient_ByScriptPath_AddsComponent()
        {
            ...
            {
                var response = server.Execute("attach_script",
                    new { target = new { instanceId = PipelineUtils.GetObjectId(m_Go) }, script = scriptPath });

                Assert.IsTrue(response.IsSuccess,
                    $"attach by script path should succeed: {response.Error} / {response.RawResponse}");
                Assert.IsNotNull(m_Go.GetComponent<AttachByPathFixture>(),
        ...
        public void GetSerializedFields_ByGameObjectAndComponent_ReturnsFields()
        {
            ...
            Assert.AreEqual("Rigidbody", (string)json["type"],
                "Resolved object should be the Rigidbody, not the GameObject");
        ...
        public void SetSerializedField_ByGameObjectAndComponent_ViaClient_RoundTrips()
        {
            ...
            using (var server = new PipelineTestServer())
            {
                ...
                Assert.IsTrue(setResponse.IsSuccess,
                    $"set by GO+component should succeed: {setResponse.Error} / {setResponse.RawResponse}");
    ...
    class ScriptCommandTestBehaviour : MonoBehaviour
    {
        ...
        [SerializeField]
        private GameObject[] m_Waypoints = Array.Empty<GameObject>();
