    ...
    class RunScriptCommandTests
    {
        ...
        private string WriteTempScript(string source)
        {
            var path = Path.Combine(Path.GetTempPath(),
                "run_script_test_" + System.Guid.NewGuid().ToString("N") + ".cs");
...
public static class Builder
{
    public static async Task<int> Build()
    {
        ...
        {
            var path = Path.Combine(Path.GetTempPath(),
                "run_script_missing_" + System.Guid.NewGuid().ToString("N") + ".cs");
...
public static class Builder
{
    public static string Build()
    {
        ...
        return string.Join("","", parts);
            ...
            Assert.AreEqual("editor,flag", withFlag.Result,
                "a custom define must extend, not replace, the active defines");
        }
        #endregion
...
public static class Builder
{
    public static int Build() { return 11; }
        ...
        public async Task RunScript_Hotpatch_MissingFile_MapsReloadFileFailure()
        {
            ...
            var path = Path.Combine(Path.GetTempPath(),
                "run_script_hotpatch_missing_" + System.Guid.NewGuid().ToString("N") + ".cs");
        ...
        public async Task RunScript_Hotpatch_DryRun_Rejected_NothingApplied()
        {
            ...
            var path = Path.Combine(Path.GetTempPath(),
                "run_script_hotpatch_dryrun_" + System.Guid.NewGuid().ToString("N") + ".cs");
        ...
        public async Task RunScript_Hotpatch_EntrySupplied_Rejected()
        {
            ...
            var path = Path.Combine(Path.GetTempPath(),
                "run_script_hotpatch_entry_" + System.Guid.NewGuid().ToString("N") + ".cs");
        ...
        {
            var path = Path.Combine(Path.GetTempPath(),
                "run_script_hotpatch_args_" + System.Guid.NewGuid().ToString("N") + ".cs");
...
public static class Builder
{
    public static string Build(int i, string[] names) { return i + "":"" + names.Length; }
            ...
            using (var server = new PipelineTestServer())
            {
                var response = server.Execute("run_script",
                    new { file, entry = "Builder.Build", args = new object[] { 5, new[] { "a", "b", "c" } } });
