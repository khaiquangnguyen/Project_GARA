...
namespace Unity.Pipeline.Tests.Editor
{
    ...
    class JobEndpointTests
    {
        ...
        [CliCommand("job_test_delayed_progress", "Test command: wait for release, then report progress",
        ...
        [CliCommand("job_test_cancellable", "Test command: loop until cooperatively canceled",
            MainThreadRequired = false)]
        ...
        {
            var response =
                await m_PipelineClient.PostJsonAsync("/api/exec", new { command, parameters = new { }, job = true });
        ...
        public async Task JobResponse_IncludesNullsByDefault_OmitNullsParamDropsThem()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(httpResponse.IsSuccessStatusCode,
                    $"/api/job with omit_nulls should succeed, got: {httpResponse.StatusCode}");
        ...
        public async Task OmitNullsBareFlag_YieldsWarningInsteadOfSilentFalse()
        {
            ...
            try
            {
                ...
                StringAssert.Contains("omit_nulls given without a value ignored; use omit_nulls=true",
                    warnings[0]?.ToString());
            }
            finally
            {
        ...
        public async Task Eval_AcceptsTimeoutAboveThirtySeconds()
        {
            ...
            var response =
                await m_PipelineClient.ExecuteCommandAsync("eval", new { code = "return 21 * 2;", timeout = 120000 });
        ...
        public async Task DetachedJob_UnconvertibleParameter_KeepsParameterValidationCategory()
        {
            ...
            var response = await m_PipelineClient.PostJsonAsync("/api/exec",
                new { command = "console", parameters = new { tail = "not-a-number" }, job = true });
