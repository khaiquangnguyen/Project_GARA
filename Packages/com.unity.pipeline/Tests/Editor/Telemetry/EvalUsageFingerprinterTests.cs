...
namespace Unity.Pipeline.Tests.Editor.Telemetry
{
    ...
    class EvalUsageFingerprinterTests
    {
        ...
        [TestCase("PlayerSettings.SetScriptingBackend(BuildTargetGroup.Standalone, ScriptingImplementation.IL2CPP);",
        ...
        public void GenericMethod_DifferentTypeArgs_ShareOneFingerprint()
        {
            ...
            Assert.AreEqual(
                Fingerprints("Object.FindFirstObjectByType<Player>();")
                    .Single(f => f.StartsWith("Object.FindFirstObjectByType")),
                Fingerprints("Object.FindFirstObjectByType<Enemy>();")
                    .Single(f => f.StartsWith("Object.FindFirstObjectByType")));
        }
        ...
        public void CastReceiver_UnwrapsToInnerExpression()
        {
            ...
            Assert.IsFalse(fps.Any(f => f.Contains("Foo")),
                $"The cast's type name must be unwrapped away: [{string.Join(", ", fps)}]");
        ...
        private static void AssertNoSourceLeak(List<string> fps, params string[] literalFragments)
        {
            foreach (var fp in fps)
            {
                ...
                foreach (var fragment in literalFragments)
                    StringAssert.DoesNotContain(fragment, fp,
                        "Fingerprints must never contain literal/source fragments");
            }
        }
