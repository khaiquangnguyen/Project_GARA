...
namespace Unity.Pipeline.Tests.Editor.Console
{
    ...
    class EditorConsoleGroundTruthTests
    {
        ...
        public void Sample_PublishesConsoleCountsAndCompilationFlag()
        {
            ...
            Assert.Less((DateTime.UtcNow - truth.SampledUtc).TotalSeconds, 30,
                "The snapshot should be the one just taken");
        }
        ...
        public void Seed_RestoresConsoleFlagsAndFilterText()
        {
            ...
            try
            {
                ...
                Assert.AreEqual(restrictedFlags, GetConsoleFlags(),
                    "Seeding must leave the console's level toggles as it found them");
                Assert.AreEqual(restrictedFilter, GetFilteringText(),
                    "Seeding must leave the console's search text as it found it");
            }
            finally
            {
        ...
        static void SetConsoleFlags(int flags) =>
            Property("consoleFlags").GetSetMethod(true).Invoke(null, new object[] { flags });
        ...
        {
            var method =
                typeof(ConsoleLogCapture).GetMethod("OnLogMessage", BindingFlags.NonPublic | BindingFlags.Static);
            Assert.IsNotNull(method,
                "ConsoleLogCapture.OnLogMessage must exist for this test to strip its subscription");
