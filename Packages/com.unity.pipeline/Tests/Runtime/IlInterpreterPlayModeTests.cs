...
public static class Probe {{ public static object Run() {{ {body} }} }}";
            ...
            {
                SourceCode = source, AssemblyName = "IlInterpreterPlayModeProbe", SkipLoad = true,
            });
            Assert.IsTrue(compile.Success, "probe source should compile");

            using var interp =
                new IlInterpreter.Interpreter.ScriptInterpreter(IlInterpreterHostBindings.CreateStandard());
