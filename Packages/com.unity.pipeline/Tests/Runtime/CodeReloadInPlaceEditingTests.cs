...
namespace Unity.Pipeline.Tests.Runtime
{
    ...
    /// </summary>
    [Ignore(
        "CodeReload in-place editing is deferred until the autonomous test loop is solid; this path exercises the known instance-to-static transformation bug. Re-enable when revisiting in-place reload.")]
...
public class CodeReloadExampleComponent : MonoBehaviour
{
    ...
    public void ResetPosition()
    {
        transform.position = Vector3.zero;
            ...
            Assert.IsTrue(
                task.Result.TransformedCode.Contains(
                    "[CodeReloadOverrideMethod(\"CodeReloadExampleComponent.Update\")]"));
            Assert.IsTrue(
                task.Result.TransformedCode.Contains(
                    "[CodeReloadOverrideMethod(\"CodeReloadExampleComponent.CalculateDamage\")]"));
            Assert.IsTrue(
                task.Result.TransformedCode.Contains(
                    "[CodeReloadOverrideMethod(\"CodeReloadExampleComponent.ResetPosition\")]"));
        }
...
public class CodeReloadExampleComponent : MonoBehaviour
{
    ...
    void Update()
    {
        this.transform.position += Vector3.right * this.speed * Time.deltaTime;
            ...
            Assert.IsTrue(transformedCode.Contains("instance."),
                "Transformed code should contain 'instance.' references");
            Assert.IsTrue(transformedCode.Contains("instance.transform"),
                "Should convert this.transform to instance.transform");
...
public class CodeReloadExampleComponent : MonoBehaviour
{
    ...
    public void MoveTowards(Vector3 target, float customSpeed)
    {
        ...
        transform.position += direction * customSpeed * Time.deltaTime;
            ...
            Assert.IsTrue(
                transformedCode.Contains(
                    "MoveTowards(CodeReloadExampleComponent instance, Vector3 target, float customSpeed)"),
...
public class TestComponent : MonoBehaviour
{
    ...
    void Update() { }
            ...
            {
                // Act
                var result = InPlaceReloadProcessor.ContainsCodeReloadableMethodsAsync(testPath).GetAwaiter()
                    .GetResult();
